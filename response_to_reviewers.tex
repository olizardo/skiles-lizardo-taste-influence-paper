...
\textbf{Location in Revised Manuscript:} Throughout the manuscript, Section 2.3, Section 3.1, and Section 3.2)
...
To document the sample composition transparently, we have added Table 1 in Section 2.1, displaying the demographic characteristics of the primary analytic sample alongside U.S. Census benchmarks and across experimental treatment and control conditions:
...
In particular, addressing the reviewer's query regarding whether the reference group was framed as possessing the \textit{same} or \textit{different} status relative to the respondent, we clarify that the experimental vignettes \textit{never described the reference group in relative terms} (i.e., participants were never told that others had ``the same status as you'' or ``higher status than you''). Instead, the survey provided descriptions of the fictional other's educational credentials and occupational positions. In the Taste-Only condition, no socioeconomic information was provided (``Other people who have taken this survey recently have said that they [LIKE / DISLIKE] the painting you saw''). The complete text and response scales for Trial 1, the intervening demographic and musical modules, and Trial 2 are now fully documented in Appendix B.
...
\textbf{Response:}
We have revised Sections 2.1 and 2.6 to clarify the exact comparisons evaluated in our balance diagnostics. One issue is that there are two forms of ``balancing'' across experimental conditions and the ``balancing'' done by the IPW weights described earlier.
