...
Repeated exposure to a cultural object across trials is expected to produce an upward drift in evaluation even in the absence of external social information \citep{zajonc1968attitudinal, reber1998effects-efe}. External social cues either facilitate or suppress this exposure drift.

\textit{Generalized Influence:}\\
Exposure to generalized positive peer evaluations facilitates positive exposure drift, whereas exposure to generalized negative peer evaluations exerts an asymmetric veto, suppressing positive exposure drift.

\textit{Status-Differentiated Influence:}\\
\textit{Hypothesis 1:} Individuals adjust their judgments to align with the aggregate evaluations of their status peers (shifting positively for peer likes and negatively for peer dislikes).

\textit{Hypothesis 2:} Lower-status respondents align their judgments with high-status peers, exhibiting cultural goodwill toward high-status evaluations.

\textit{Hypothesis 3:} Individuals distance themselves from out-group judgments, shifting away from the evaluations of socially dissimilar others.

\textit{Status Consistency and Behavioral Inertia:}\\
\textit{Hypothesis 4:} Individuals with consistent status profiles (e.g., Middle Class with College, or Working Class without College) exhibit higher propensities to maintain their initial evaluation (``staying'') compared to status-inconsistent individuals.
...
\begin{enumerate}
    ...
    \item \textbf{Intervening Survey Block \& Manipulation:} Respondents completed demographic questions and a musical taste battery. Treatment respondents then read a brief vignette describing how a peer group evaluated the artwork.
...
To solve this problem and strengthen causal inference, we implement Inverse Probability Weighting (IPW) using generalized propensity scores \citep{rosenbaum1983central}. The logic of IPW proceeds in two steps:
\begin{enumerate}
...
To strictly isolate information-induced shifts from repeated-measurement exposure drift, we formulate our analysis within a Difference-in-Differences (DiD) framework. Rather than examining simple pre-to-post changes within each condition in isolation—which conflates the effect of peer information with the natural exposure drift that occurs upon viewing an artwork a second time—the DiD strategy benchmarks every treatment condition against the unexposed Control Condition.

We specify a linear mixed-effects model with random intercepts for respondents:
\begin{equation}
\text{Taste}_{ijt} = \beta_0 + \beta_{\text{Trial}} \text{Trial}_{2,it} + \sum_{k=1}^{K} \beta_{k} \text{Cond}_{ik} + \sum_{k=1}^{K} \delta_{k} (\text{Trial}_{2,it} \times \text{Cond}_{ik}) + \mathbf{X}_i \boldsymbol{\gamma} + u_i + \epsilon_{ijt}
\label{eq:did_model}
\end{equation}
where $i$ indexes individual respondents, $j$ indexes observations, $t \in \{1, 2\}$ indexes trials, and the unexposed Control Condition serves as the omitted reference category for the experimental conditions $\text{Cond}_{ik}$.

To provide a clear pedagogical interpretation of the parameters in Equation \ref{eq:did_model}:
\begin{enumerate}
    \item \textbf{The Intercept ($\beta_0$):} Represents the expected baseline aesthetic evaluation at Trial 1 (pre-treatment) for respondents assigned to the unexposed Control Condition:
    \begin{equation}
    \beta_0 = E[\text{Taste} \mid \text{Trial} = 1, \text{Control}]
    \end{equation}
    \item \textbf{The Trial 2 Main Effect ($\beta_{\text{Trial}}$):} Captures the counterfactual \textit{exposure drift} from Trial 1 to Trial 2 in the absence of any external peer information:
    \begin{equation}
    \beta_{\text{Trial}} = E[\text{Taste} \mid \text{Trial} = 2, \text{Control}] - E[\text{Taste} \mid \text{Trial} = 1, \text{Control}]
    \end{equation}
    If repeated viewing induces an intrinsic appreciation of the artwork (the mere exposure effect), $\beta_{\text{Trial}}$ will be positive and statistically significant.
    \item \textbf{Condition Main Effects ($\beta_k$):} Reflect pre-treatment differences between condition $k$ and the Control Condition at Trial 1:
    \begin{equation}
    \beta_k = E[\text{Taste} \mid \text{Trial} = 1, \text{Cond} = k] - E[\text{Taste} \mid \text{Trial} = 1, \text{Control}]
    \end{equation}
    Because respondents were randomly assigned to experimental conditions before viewing the painting, these coefficients assess baseline balance and are expected to be close to zero.
    \item \textbf{The Interaction Coefficients ($\delta_k$):} Represent the \textit{causal Difference-in-Differences treatment effects}. Each $\delta_k$ measures whether the change in evaluations from Trial 1 to Trial 2 in treatment condition $k$ differed significantly from the change observed in the unexposed Control Condition:
    \begin{equation}
    \delta_k = \left( E[\text{Taste} \mid \text{Trial} = 2, k] - E[\text{Taste} \mid \text{Trial} = 1, k] \right) - \left( E[\text{Taste} \mid \text{Trial} = 2, \text{Control}] - E[\text{Taste} \mid \text{Trial} = 1, \text{Control}] \right)
    \end{equation}
    Substantively, $\delta_k$ directly answers the counterfactual question: \textit{Did exposure to peer evaluation $k$ cause respondents' aesthetic judgments to change significantly more or less than they would have changed under mere repeated exposure alone?}
    \begin{itemize}
        \item A positive estimate ($\delta_k > 0$) indicates that peer feedback facilitated aesthetic appreciation beyond the natural exposure drift.
        \item A negative estimate ($\delta_k < 0$) indicates that peer feedback suppressed or reversed the positive exposure drift that would have naturally occurred.
    \end{itemize}
\end{enumerate}
...
\end{equation}
where the intercept directly estimates the control exposure drift ($\beta_{\text{Trial}}$) and the condition slopes estimate the DiD treatment effects ($\delta_k$). To examine non-linear behavioral choices, we also estimate multinomial logistic regressions modeling the discrete probabilities of Staying, Conforming, and Reacting.
...
The behavioral model provides strong support for \textbf{Hypothesis 4}:
\begin{itemize}
    \item \textbf{Status Consistency Promotes Evaluative Inertia:} Evaluative stability (``staying'') is highest among status-consistent individuals in both high and low positions (Middle Class/College: 79.0\%; Working Class/No College: 74.4\%). Both groups occupy clear structural locations, giving them high evaluative certainty.
    \item \textbf{Status Inconsistency Produces Evaluative Fluidity:} In contrast, status-inconsistent individuals display lower inertia and higher rates of conformity (Working Class/College: 19.0\%; Middle Class/No College: 17.8\%).
    \item \textbf{Educational Reactance:} Reactance is concentrated among non-college-educated individuals (12.0\%--13.3\%), who are more likely to move in the opposite direction of external social cues.
\end{itemize}
