...
With respondents without a college degree serving as the reference category.

The model reveals that the counterfactual exposure drift across trials for respondents without a college degree is statistically indistinguishable from zero ($\beta_1 = +0.032, SE = 0.058, t = 0.55, p = 0.582$). In contrast, the interaction between Trial and College degree is positive and statistically significant ($\beta_2 = +0.247, SE = 0.082, t = 3.00, p = 0.0027^{**}$). Combining these parameters demonstrates that college-educated respondents exhibit substantial upward exposure drift ($\text{Total Drift} = +0.279, SE = 0.063, t = 4.43, p < 0.001$). Thus, after balancing baseline demographic covariates, repeated exposure elicits an intrinsic aesthetic appreciation specifically among respondents with institutionalized cultural capital, whereas non-college respondents maintain static evaluations.
...
\addlinespace[4pt]
\textit{Valence Asymmetry} & Negative cues from others halt exposure drift; positive cues facilitate drift & High-Status Dislike: $\text{DiD} = -0.121$ ($p = 0.043^{*}$);\newline Generalized Dislike: $\text{DiD} = -0.125$ ($p = 0.095^{\dagger}$);\newline Positive cues: null ($p \ge 0.198$) & Supported ($p < 0.05$) \\
\addlinespace[4pt]
