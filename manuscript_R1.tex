...
Inspecting the separate models reveals that education is the primary structural fault line governing responsiveness to external evaluations (Panel A of Figure \ref{fig:did_edu_class} and Table \ref{tbl:did_edu_class}). Non-college respondents tend to either maintain or positively shift their evaluations. In particular, non-college degree respondents align with an unmarked majority in terms of likes ($\text{DiD} = +0.228, SE = 0.100, p = 0.02$) and are marginally likely ($p < 0.10$), consistent with the cultural goodwill thesis, to align positively when told that high-status people liked the painting. Puzzlingly, and not predicted by any theory, people without a college degree tend to react to the painting when informed that low-status people \textit{dislike} it by increasing their evaluation of it ($\text{DiD} = +0.198, SE = 0.088, p = 0.025^{*}$).
...
Table \ref{tbl:did_status_quadrants} presents the complete 24-cell matrix of Difference-in-Differences treatment effect estimates across all experimental conditions and all four status consistency quadrants, estimated using Inverse Probability Weighting (IPW) and benchmarked against the unexposed Control Condition. Figure \ref{fig:did_status_full} in the Appendix displays the corresponding point estimates and 95\% confidence intervals.
...
\vspace{4pt}
\footnotesize
\textit{Note:} $^{\dagger}p < 0.10, ^{*}p < 0.05, ^{**}p < 0.01, ^{***}p < 0.001$. Standard errors in parentheses. Estimates represent Difference-in-Differences treatment contrasts relative to the unexposed Control Condition within each status consistency quadrant, estimated using Inverse Probability Weighting (IPW) balancing baseline age, gender, race/ethnicity, and parental education.
\end{minipage}
\end{table}
...
\section{Complete Status-Differentiated DiD Estimates}
