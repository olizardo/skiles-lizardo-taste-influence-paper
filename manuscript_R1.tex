...
Accordingly, we specify a linear mixed-effects models with random intercepts for respondents of the general form:
\begin{equation}
\begin{split}
...
Instead, to test our focal theoretical hypotheses regarding how social influence is moderated by respondents' structural social location (educational attainment, subjective social class, and the four status consistency quadrants), we extend Equation \ref{eq:did_model} to a fully moderated Difference-in-Differences specification:
\begin{equation}
\begin{split}
...
We begin by establishing a status-free empirical baseline: testing simultaneously for the mere-exposure drift observed under repeated evaluation in the unexposed Control Condition and examining whether learning that unknown others liked or disliked the artwork shifts aesthetic evaluations in the aggregate via a generalized influence mechanism. To do so, we estimate a repeated-measures linear mixed model on the subset of respondents assigned to the Control Condition ($N = 177$), the Generalized Like condition ($N = 172$), and the Generalized Dislike condition ($N = 172$), totaling $N = 521$ respondents ($1,042$ observations across trials) with Inverse Probability Weighting (IPW):
\begin{equation}
\begin{split}
...
This status-free baseline model establishes two main findings. First, consistent with the mere exposure effect hypothesis (baseline), in the complete absence of external social cues (the Control Condition), repeated exposure generates a statistically significant upward appreciation of the artwork ($\beta_1 = +0.156, SE = 0.052, t = 3.00, p = 0.003$), confirming the classical mere exposure effect. 

Second, the model reveals a pronounced \textit{valence asymmetry} in generalized social influence. When respondents are informed that unknown others liked the painting, their raw evaluations shift positively from 4.51 to 4.74 ($+0.224, p < 0.001$); relative to the natural counterfactual drift of the control group, this additional increment is positive but not statistically distinguishable on its own ($\delta_{\text{Like}} = +0.068, SE = 0.075, t = 0.90, p = 0.367$). In sharp contrast, learning that others disliked the painting completely overrides the positive mere exposure effect, flattening second-trial evaluations to a static $-0.016$ ($p = 0.758$). Relative to the control baseline drift, this constitutes a statistically significant negative Difference-in-Differences suppression ($\delta_{\text{Dislike}} = -0.172, SE = 0.074, t = -2.35, p = 0.02$). Directly contrasting positive and negative cues reveals a significant directional swing of nearly a quarter point on the 7-point scale ($\Delta = +0.240, SE = 0.075, \chi^2 = 10.32, p = 0.001$), demonstrating that \textit{negative} generalized social signals are much stronger than positive ones in the absence of any further information.

While this status-free model demonstrates clear valence asymmetry overall, evaluating social influence solely at the aggregate level conceals substantial underlying heterogeneity across social locations (as shown in the full pooled models in Appendix Table A.2 and Appendix Figure A.2). We therefore turn to our primary moderated models to examine how social influence is conditioned by educational capital, subjective social class, and structural status consistency.

\subsection{Social Influence Moderated by Education and Subjective Class}
\label{sec:stepwise_models}
To examine how social influence operates across distinct dimensions of class and status difference, we first inspect how educational attainment alone (College Degree vs.\ No College) and subjective social class alone (Working Class vs.\ Middle Class) moderate the experimental treatments, using a model based on Equation~\ref{eq:did_moderation_model}. Figure \ref{fig:did_edu_class} visualizes these treatment effects with 95\% confidence intervals compared against the unexposed Control Condition (complete regression estimates are reported in Appendix Table \ref{tbl:did_edu_class}).
...
Inspecting the separate models reveals that education is the primary structural fault line governing responsiveness to external evaluations (Panel A of Figure \ref{fig:did_edu_class} and Appendix Table \ref{tbl:did_edu_class}). Non-college respondents tend to either maintain or positively shift their evaluations. In particular, non-college degree respondents align with an unmarked majority in terms of likes ($\text{DiD} = +0.228, SE = 0.100, p = 0.02$) and are marginally likely ($p < 0.10$), consistent with the cultural goodwill thesis, to align positively when told that high-status people liked the painting. Puzzlingly, and not predicted by any theory, people without a college degree tend to react to the painting when informed that low-status people \textit{dislike} it by increasing their evaluation of it ($\text{DiD} = +0.198, SE = 0.088, p = 0.025^{*}$).

In sharp contrast, college-educated respondents—whose positive baseline exposure drift was established in Section \ref{sec:stepwise_models}—experience substantial evaluative suppression from external negative cues: High-Status Dislike ($\text{DiD} = -0.275, SE = 0.094, p = 0.003^{**}$), Generalized Dislike ($\text{DiD} = -0.302, SE = 0.118, p = 0.010^{*}$), and Low-Status Dislike ($\text{DiD} = -0.226, SE = 0.101, p = 0.026^{*}$). In addition, college graduates exhibit Bourdieusian taste abandonment when exposed to Low-Status Likes ($\text{DiD} = -0.248, SE = 0.104, p = 0.017^{*}$).
