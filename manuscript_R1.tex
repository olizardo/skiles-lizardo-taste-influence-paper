...
Inspecting the separate models reveals that educational capital is the primary structural fault line governing responsiveness to external evaluations (Panel A of Figure \ref{fig:did_edu_class} and Table \ref{tbl:did_edu_class}). Non-college respondents exhibit positive shifts under Generalized Likes ($\text{DiD} = +0.228, SE = 0.100, p = 0.022^{*}$) and Low-Status Dislikes ($\text{DiD} = +0.198, SE = 0.088, p = 0.025^{*}$), while remaining largely unresponsive to high-status cues. In sharp contrast, college-educated respondents—whose positive baseline exposure drift was established in Section \ref{sec:stepwise_models}—experience substantial evaluative suppression from external negative cues: High-Status Dislike ($\text{DiD} = -0.275, SE = 0.094, p = 0.003^{**}$), Generalized Dislike ($\text{DiD} = -0.302, SE = 0.118, p = 0.010^{*}$), and Low-Status Dislike ($\text{DiD} = -0.226, SE = 0.101, p = 0.026^{*}$). In addition, college graduates exhibit Bourdieusian taste abandonment when exposed to Low-Status Likes ($\text{DiD} = -0.248, SE = 0.104, p = 0.017^{*}$).
