...
\begin{figure}[ht!]
    ...
    \caption{Statistically Reliable Difference-in-Differences Treatment Effects ($p < 0.10$ and $p < 0.05$). Points represent point estimates and error bars denote 95\% confidence intervals relative to the unexposed Control Condition (dashed vertical line at zero). Colors indicate statistical significance: red ($p < 0.05$) and orange ($p < 0.10$). The full 24-cell grid including non-significant estimates is displayed in Appendix Figure \ref{fig:did_status_full}.}
...
Figure \ref{fig:did_status_full} displays the complete 24-cell grid of Difference-in-Differences treatment effect estimates across all six treatment conditions and all four status consistency quadrants, estimated using Inverse Probability Weighting (IPW). Points indicate point estimates and error bars denote 95\% confidence intervals relative to the unexposed Control Condition within each status group.
