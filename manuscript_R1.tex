...
While continuous change models identify average shifts, they mask non-linear behavioral choices where opposing movements cancel out in the aggregate. To examine these choices, we categorize responses into Stay ($\Delta = 0$), Conform (moving toward others' evaluation), and React (moving away from others' evaluation). Figure \ref{fig:multinom} presents the average marginal effects (differences in choice probabilities) from multinomial logistic regression, benchmarked against the reference category of Working Class individuals without a college degree.

\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/Figure7_MultinomialBehavior.png}
    \caption{Average Marginal Effects on Behavioral Choices (Stay, Conform, React) Across Status Consistency Groups. Horizontal bars indicate differences in probability relative to the reference group (Working Class, No College), with 95\% confidence intervals. Colors indicate statistical significance: red ($p < 0.05$) and grey ($p \ge 0.10$).}
    \label{fig:multinom}
\end{figure}

The marginal effects provide strong empirical support for \textit{Hypothesis 4}. In terms of evaluative stability (Panel A: Stay), Middle Class individuals with a college degree are significantly more likely to stay with their initial evaluation compared to non-college workers ($\text{AME} = +0.046, SE = 0.013, p < 0.001$). Conversely, Middle Class individuals without a college degree exhibit significantly lower stability ($\text{AME} = -0.054, SE = 0.013, p < 0.001$). This shows that structural consistency fosters evaluative certainty, whereas status inconsistency produces evaluative fluidity.

In terms of conformity (Panel B), status-inconsistent individuals are significantly more prone to shift toward others' opinions. Working Class individuals with a college degree are $+5.4\%$ more likely to conform ($\text{AME} = +0.054, SE = 0.011, p < 0.001$), and Middle Class individuals without college are $+4.1\%$ more likely to conform ($\text{AME} = +0.041, SE = 0.011, p < 0.001$). In contrast, high-status consistent individuals are significantly less likely to conform ($\text{AME} = -0.021, SE = 0.010, p = 0.035$).

Finally, in terms of reactance (Panel C), college-educated individuals—regardless of subjective class—are significantly less likely to react against others' evaluations compared to non-college workers ($\text{AME} = -0.032, SE = 0.009, p < 0.001$ for Working Class/College; $\text{AME} = -0.025, SE = 0.009, p = 0.007$ for Middle Class/College). This shows that reactance is concentrated among non-college-educated individuals.
