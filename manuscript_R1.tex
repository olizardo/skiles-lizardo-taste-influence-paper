...
Looking at the separate models reveals that education is the primary structural fault line governing responsiveness to external evaluations when evaluators are differentiated by status  (Panel A of Figure \ref{fig:did_edu_class} and Appendix Table \ref{tbl:did_edu_class}). Non-college respondents tend to either maintain or positively shift their evaluations. In particular, non-college degree respondents align with an unmarked majority in terms of likes ($\text{DiD} = +0.228, SE = 0.100, p = 0.02$) and are marginally likely ($p < 0.10$), consistent with the cultural goodwill thesis, to align positively when told that high-status people liked the painting. Puzzlingly, and contrary to the predictions of any theory, people without a college degree tend to react to the painting when informed that low-status people \textit{dislike} it, increasing their evaluation of it ($\text{DiD} = +0.198, SE = 0.088, p = 0.03$).

In contrast, college-educated respondents are statistically significantly more likely to revise their evaluations downward, but never upward, across four of the six experimental conditions. In particular, college-educated respondents seem to experience both general aversions to people disliking the painting regardless of status, responding negatively in the High-Status Dislike condition ($\text{DiD} = -0.275, SE = 0.094, p = 0.003$), the Low-Status Dislike condition ($\text{DiD} = -0.226, SE = 0.101, p = 0.03$, and the Generalized Dislike condition $\text{DiD} = -0.302, SE = 0.118, p = 0.01$). In addition, college graduates exhibit Bourdieusian distancing behavior, lowering their regard for the painting when told that Low-Status people liked it ($\text{DiD} = -0.248, SE = 0.104, p = 0.02$). Thus, the college-educated seem to be sensitive to dislike information, but also engage in Bourdieusian distancing behavior. 

By comparison, examining subjective social class without regard to education (Panel B of Figure \ref{fig:did_edu_class}) produces a more diffuse picture. Self-identified working-class respondents show no statistically reliable DiD adjustments across any condition, which may be masking substantial underlying heterogeneity. Middle-class self-identifiers display significant negative shifts under High-Status Dislike ($\text{DiD} = -0.182, SE = 0.086, p = 0.035$) and Low-Status Like ($\text{DiD} = -0.202, SE = 0.095, p = 0.034$)---thus showing the same sensitivity to dislike information as college-educated people---but these effects are attenuated relative to the education-stratified models.
...
\midrule
\textit{Mere Exposure Drift (Control)} & Upward drift in evaluation across repeated trials without external social cues & $\beta_1 = +0.156$ ($p = 0.0029^{**}$) & Supported ($p < 0.01$) \\
\addlinespace[4pt]
\textit{Valence Asymmetry} & Negative cues from others halt exposure drift; positive cues facilitate drift & High-Status Dislike: $\text{DiD} = -0.201$ ($p = 0.001^{**}$);\newline Generalized Dislike: $\text{DiD} = -0.172$ ($p = 0.024^{*}$);\newline Positive cues: null ($p \ge 0.367$) & Supported ($p < 0.05$) \\
\addlinespace[4pt]
\textit{Hypothesis 1: Same-Status Alignment} & Individuals align judgments with aggregate evaluations of others sharing their status & High-status like: $\text{DiD} = -0.118$ ($p = 0.348$);\newline Low-status like: $\text{DiD} = -0.110$ ($p = 0.404$) & Not Supported ($p \ge 0.10$) \\
\addlinespace[4pt]
\textit{Hypothesis 2: Symbolic Power} & Lower-status individuals align judgments with high-status others & Low-status consistent like: $\text{DiD} = +0.130$ ($p = 0.256$);\newline Working/College dislike: $\text{DiD} = -0.394$ ($p = 0.001^{**}$) & Not Supported for Low-Status Consistent ($p \ge 0.10$) \\
\addlinespace[4pt]
\textit{Hypothesis 3: Cross-Status Reactance} & Individuals distance judgments from outgroup evaluations & High-status exposed to Low-Status Like: $\text{DiD} = -0.296$ ($p = 0.039^{*}$);\newline Low-Status Dislike: $\text{DiD} = -0.227$ ($p = 0.097^{\dagger}$) & Supported ($p < 0.05$) \\
\addlinespace[4pt]
...
\vspace{4pt}
\footnotesize
\textit{Note:} Difference-in-Differences estimates control treatment shifts against the unexposed Control Condition. Status-subgroup estimates and behavioral models incorporate Inverse Probability Weighting (IPW) on age, gender, race/ethnicity, and parental education.
\end{minipage}
\end{table}
...
\midrule
Control Exposure Drift & -0.013 (0.099) & +0.311$^{**}$ (0.104) & +0.098 (0.120) & +0.241$^{*}$ (0.111) \\
...
\vspace{4pt}
\footnotesize
$^{\dagger}p < 0.10, ^{*}p < 0.05, ^{**}p < 0.01, ^{***}p < 0.001$. Standard errors in parentheses. \newline \textit{Note:} Estimates represent Difference-in-Differences treatment contrasts relative to the unexposed Control Condition within each status consistency quadrant, estimated using Inverse Probability Weighting (IPW) balancing baseline age, gender, race/ethnicity, and parental education.
\end{minipage}
\end{table}
