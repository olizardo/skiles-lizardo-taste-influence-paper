...
The experiment used an incomplete factorial design with within-subject pre- and post-trials. In Trial 1 (pre-treatment baseline), respondents viewed the artwork accompanied by the verbatim prompt:
\begin{quote}
...
\subsection{Primary Main Models: Social Influence Moderated by Education and Subjective Class}
...
Inspecting the separate models reveals that education is the primary structural fault line governing responsiveness to external evaluations (Panel A of Figure \ref{fig:did_edu_class} and Table \ref{tbl:did_edu_class}). Non-college respondents tend to either maintain or positively shift their evaluations. In particular, non-college degree respondents align with an unmarked majority in terms of likes ($\text{DiD} = +0.228, SE = 0.100, p = 0.022^{*}$) and are marginally likely ($p < 0.10$), consistent with the cultural goodwill thesis, to align positively when told that high-status people liked the painting. Puzzlingly, and not predicted by any theory, people without a college degree tend to react to the painting when informed that low-status people \textit{dislike} it by increasing their evaluation of it ($\text{DiD} = +0.198, SE = 0.088, p = 0.025^{*}$).
