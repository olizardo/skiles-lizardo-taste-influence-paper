...
Evaluating the unexposed Control Condition allows us to observe the counterfactual drift in aesthetic judgments over repeated trials in the complete absence of cues from others. As shown in Model 1 of Table \ref{tbl:did_models}, we estimate a repeated-measures linear mixed model within the Control Condition ($N = 177$ respondents, $354$ observations) testing whether exposure drift is moderated by educational attainment:
\begin{equation}
...
For respondents without a college degree, the counterfactual exposure drift across trials is exactly zero ($\beta_1 = 0.000, SE = 0.052, t = 0.00, p = 1.000$). In contrast, the interaction between Trial and College degree is positive and statistically significant ($\beta_2 = +0.257, SE = 0.080, t = 3.21, p = 0.0016^{**}$). Adding the interaction parameter reveals that college-educated respondents exhibit substantial upward exposure drift ($\text{Total Drift} = +0.257, SE = 0.061, t = 4.22, p < 0.001$). Thus, the mere exposure effect is conditioned by educational capital: individuals with higher institutionalized cultural capital experience an intrinsic aesthetic appreciation upon re-evaluation, whereas non-college individuals maintain static evaluations.
...
\toprule
& \multicolumn{2}{c}{\textbf{Model 1: Control Drift Model}} & \multicolumn{2}{c}{\textbf{Model 2: Pooled DiD Model}} \\
\cmidrule(lr){2-3} \cmidrule(lr){4-5}
\textbf{Independent Variable} & Estimate (SE) & $t$ value & Estimate (SE) & $t$ value \\
\midrule
Intercept & 4.874 (0.155) & 31.47$^{***}$ & 4.689 (0.114) & 40.99$^{***}$ \\
Trial 2 Exposure Drift ($\beta_1$) & +0.000 (0.052) & 0.00 & +0.107 (0.052) & 2.05$^{*}$ \\
College Degree Main Effect & -0.441 (0.240) & -1.84$^{\dagger}$ & \multicolumn{2}{c}{---} \\
Trial 2 $\times$ College Degree ($\beta_2$) & +0.257 (0.080) & 3.21$^{**}$ & \multicolumn{2}{c}{---} \\
\addlinespace[3pt]
...
\midrule
Total College Exposure Drift & +0.257 (0.061) & 4.22$^{***}$ & \multicolumn{2}{c}{---} \\
Random Intercept Variance ($\sigma^2_u$) & \multicolumn{2}{c}{2.333} & \multicolumn{2}{c}{2.074} \\
Residual Variance ($\sigma^2_\epsilon$) & \multicolumn{2}{c}{0.138} & \multicolumn{2}{c}{0.243} \\
Observations / Individuals & \multicolumn{2}{c}{354 / 177} & \multicolumn{2}{c}{4,506 / 2,253} \\
Log Likelihood & \multicolumn{2}{c}{-469.3} & \multicolumn{2}{c}{-6,465.0} \\
\bottomrule
\end{tabular*}
\begin{minipage}{\linewidth}
\vspace{4pt}
\footnotesize
\textit{Note:} $^{\dagger}p < 0.10, ^{*}p < 0.05, ^{**}p < 0.01, ^{***}p < 0.001$. Models estimated via linear mixed-effects with random intercepts for respondents. Model 1 restricts the sample to the unexposed Control Condition ($N = 177$) to evaluate counterfactual exposure drift and its moderation by educational attainment (non-college respondents serve as reference). Model 2 estimates the primary Difference-in-Differences specification across the pooled sample ($N = 2,253$), benchmarking treatment shifts against the unexposed Control Condition.
\end{minipage}
\end{table}
